...
const rows = data.map(row => {
  ...
  function imprimir_todas_guias(guiasFiltradas) {
    const chunkSize = 10; // El tamaño de cada grupo
...
$("#informacion_comi").on("click", function() {
  ...
  if ( rutaSeleccionada === '') {
    console.log('Select vacío - Ejecutando nueva consulta por día de la semana');
    
    if (!fechaDesde) {
        Swal.fire({
            icon: 'warning',
            title: 'Fecha requerida',
            text: 'Por favor selecciona una fecha para consultar'
        });
        return;
    }

    // Ejecutar nueva consulta por día de la semana
    consultarGuiasPorDiaSemana(fechaDesde, fechaHasta);
    return;
}
      ...
      .done(function(guiasData) {
          ...
          if (!contenedorBotones) {
              ...
              rutasGuardadas.forEach((rutaGuardada) => { // No necesitas 'index' aquí
                  ...
                  if (usuario_nivel === 0 || usuario_nivel === 1 || usuario_nivel === 2 || usuario_nivel === 8 ) { // Asumiendo que 'usuario_nivel' y 'mi_usuario' están definidos
...
function mostrarGuiasEnModal(data) {
  const modal = document.getElementById('modalGuiasPorDia');
  const contenido = document.getElementById('contenidoGuiasPorDia');
  
  // Convertir día de la semana a texto
  const diasSemana = ['Domingo', 'Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábado'];
  const nombreDia = diasSemana[parseInt(data.diaSemana)];
  datosModalActual = data;

  console.log('🔍 Datos guardados para exportación:', datosModalActual);
      ...
      <div class="row mb-3">
          <div class="col-12">
              <h6 class="text-primary">
                  ...
                  ${nombreDia} - ${data.fecha}
              </h6>
              <p class="text-muted">
                  <i class="fas fa-route"></i> ${data.totalRutas} rutas con guías | 
                  <i class="fas fa-box"></i> ${data.totalGuias} guías totales
              </p>
          </div>
      </div>
  `;
  ...
  if (data.rutas.length === 0) {
      html += `
          <div class="alert alert-info">
              ...
              No se encontraron guías para el día ${nombreDia} (${data.fecha})
          </div>
      `;
  } else {
      data.rutas.forEach((rutaData, index) => {
              ...
              <div class="card mb-3">
                  <div class="card-header bg-light">
                      <h6 class="mb-0">
                          ...
                          Ruta ${rutaData.ruta.numero}: ${rutaData.ruta.Rutas}
                          <span class="badge bg-primary ms-2">${rutaData.totalGuias} guías</span>
                      </h6>
                  </div>
                  <div class="card-body">
          ...
          rutaData.guias.forEach(guia => {
              const hora = new Date(guia.fecha).toLocaleTimeString('es-ES', {
                  hour: '2-digit',
                  minute: '2-digit'
              });
              ...
              html += `
                  <tr>
                      <td><strong>${guia.nro_guia || 'N/A'}</strong></td>
                      <td>${guia.nom_cliente_remite || 'N/A'}</td>
                      <td>${guia.nom_cliente_destina || 'N/A'}</td>
                      <td>${guia.celular_remite || 'N/A'}</td>
                      <td>${guia.celular_destina || 'N/A'}</td>
                      <td>${guia.direccion_destina || 'N/A'}</td>
                      <td>${hora}</td>
                      <td>
                          <span class="badge bg-${getStatusColor(guia.status)}">
                              ${guia.status || 'N/A'}
                          </span>
                      </td>
                      <td>${guia.agencia}</td>
...
// ✅ NUEVA FUNCIÓN: Consulta por día de la semana (solo cuando select está vacío)
async function consultarGuiasPorDiaSemana(fecha, fechaHasta) {
  try {
      // Mostrar loading
      Swal.fire({
          title: 'Consultando...',
          text: 'Buscando rutas del día de la semana',
          allowOutsideClick: false,
          showConfirmButton: false,
          willOpen: () => {
              Swal.showLoading();
      ...
      const response = await fetch('/guia/consultar-guias-por-dia', {
          method: 'POST',
          headers: {
              'Content-Type': 'application/json',
          },
          body: JSON.stringify({
              ...
              rutaSeleccionada: '' // Siempre vacío para esta consulta
          })
      });
      ...
      if (data.success) {
          mostrarGuiasEnModal(data);
      } else {
          Swal.fire({
              icon: 'error',
              title: 'Error',
              text: data.message || 'Error al consultar las guías'
          });
      }
      ...
      Swal.fire({
          icon: 'error',
          title: 'Error',
          text: 'Error de conexión al consultar las guías'
      });
  }
}
