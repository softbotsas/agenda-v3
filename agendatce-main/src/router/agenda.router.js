...
router.get('/api/confirmations/rutas', authenticateToken, (req, res) => {
    // Por ahora retornar datos de ejemplo, después conectar con el modelo real
    res.json({
        success: true,
        data: [
            { _id: '1', nombre: 'Ruta Centro', descripcion: 'Ruta hacia el centro de la ciudad' },
            { _id: '2', nombre: 'Ruta Norte', descripcion: 'Ruta hacia el norte' },
            { _id: '3', nombre: 'Ruta Sur', descripcion: 'Ruta hacia el sur' }
        ]
    });
});

router.get('/api/confirmations/choferes', authenticateToken, (req, res) => {
    // Por ahora retornar datos de ejemplo, después conectar con el modelo real
    res.json({
        success: true,
        data: [
            { _id: '1', nombre: 'Juan Pérez', name: 'Juan Pérez' },
            { _id: '2', nombre: 'María García', name: 'María García' },
            { _id: '3', nombre: 'Carlos López', name: 'Carlos López' }
        ]
    });
});

router.get('/api/confirmations/filtrar', authenticateToken, (req, res) => {
    const { fecha, ruta, chofer } = req.query;
    ...
    const data = [
        {
            ...
            status: 'Confirmado',
            comentario: 'Confirmación exitosa',
            monto: 150.00,
            registradoPor: 'Admin'
        },
        {
            ...
            status: 'Pendiente',
            comentario: 'Esperando confirmación',
            monto: 200.50,
...
// Rutas de Reconfirmaciones
router.get('/api/reconfirmations/rutas', authenticateToken, (req, res) => {
    // Por ahora retornar datos de ejemplo, después conectar con el modelo real
    res.json({
        success: true,
        data: [
...
router.get('/api/reconfirmations/choferes', authenticateToken, (req, res) => {
    // Por ahora retornar datos de ejemplo, después conectar con el modelo real
    res.json({
        success: true,
        data: [
...
router.get('/api/reconfirmations/filtrar', authenticateToken, (req, res) => {
    const { fecha, ruta, chofer } = req.query;
    ...
    const data = [
        {
            nroGuia: 'G003',
            remitente: 'Empresa DEF',
            status: 'Reconfirmado',
            comentario: 'Reconfirmación exitosa',
            monto: 175.25,
            registradoPor: 'Admin'
        },
        {
            nroGuia: 'G004',
            remitente: 'Empresa GHI',
            status: 'Pendiente',
            comentario: 'Esperando reconfirmación',
            monto: 300.75,
            registradoPor: 'Supervisor'
        }
    ];
