...
const registerAction = async (req, res) => {
  try {
    ...
    const userId = req.user._id;
    const evidenceFile = req.file ? req.file.filename : null;
    const evidenceUrl = req.body.evidence_url; // Nueva línea para procesar URL de evidencia

    console.log('🔍 registerAction controller:', { assignment_id, userId, value, comment, evidenceFile, evidenceUrl });
    ...
    const result = await TaskServiceSimple.registerActionSimple(assignment_id, userId, value, comment, evidenceFile, req, evidenceUrl);
